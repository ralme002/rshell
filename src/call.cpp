...
{
    int currentrun=0;
    //string MAINTEMP = "THW.txt";
    string OUTTEMP = "outtemp.txt";
    hardexit = 0;
...
void call::InputFile(string tmp)
{
    ...
    //cout << "after  ==" << tmp << endl;
    inFile=open(array[0], O_RDONLY);

    if(-1 == inFile)
    {
        cout << "Open failed"<< endl;
    }
    //cout << "HERE ==" << inFile << endl;
    dup2(inFile, STDIN_FILENO);
    close(inFile);
}

void call::OutFile(string tmp)
{
    save_stdout=dup(1);
     outFile=open(tmp.c_str(), O_WRONLY | O_TRUNC | O_CREAT, S_IRUSR | S_IRGRP | S_IWGRP | S_IWUSR | S_IXUSR | S_IXGRP);
     dup2(outFile, 1);
    close(outFile);
}

void call::Close_Outfile()
{
    //restores file for return output
    dup2(save_stdout, 1);
    close(save_stdout);
}
void call::Close_Infile()
{
    //restores file for return input
    dup2(save_stdin, 0);
    close(save_stdin);
}

void call::Close_In_Out_File()
{
    dup2(save_stdin,0);
    close(save_stdin);
    dup2(save_stdout,1);
...
void call::OutFile_Overwrite(string tmp)
{
    save_stdout=dup(1);
    outFile= open(tmp.c_str(), O_WRONLY | O_APPEND | O_CREAT, S_IRUSR | S_IRGRP | S_IWGRP | S_IWUSR | S_IXUSR | S_IXGRP);
    dup2(outFile, 1);
    close(outFile);
}

void call::pipe_command(string tmp)
{
    int fd[2];
    if(pipe(fd)==-1){cout << "pipe failed"<< endl;}
